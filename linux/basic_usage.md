...
- `du -sh *` 查看当前目录下各文件/目录的大小（人类可读）
- `du -sh /path` 查看指定目录总大小
- `du -ah --max-depth=1` 显示当前目录下一层的所有文件大小
- `du -s * | sort -rn | head -10` 查看当前目录下占用空间最大的前10个
- `du --exclude='*.log' -sh *` 排除特定类型文件

```bash
du -sh /var/log     # 查看日志目录总大小
du -ah . | sort -rh | head -20  # 找出当前目录最大的20个文件
```
...
- `lsblk -o NAME,SIZE,TYPE,MOUNTPOINT,FSTYPE` 自定义输出列
- `lsblk -d` 仅显示磁盘，不显示分区
- `lsblk -b` 以字节显示大小
...
- `blkid -s UUID` 仅显示UUID
- `blkid -s TYPE` 仅显示文件系统类型

```bash
blkid /dev/sda1
# /dev/sda1: UUID="xxxx-xxxx" TYPE="xfs" PARTUUID="xxxx"
```

常用于编辑 /etc/fstab 时获取分区UUID
...
# fdisk（MBR分区）
fdisk -l              # 列出所有磁盘分区信息
fdisk /dev/sda        # 进入交互式分区工具
# 常用交互命令：n(新建) d(删除) p(打印) w(写入) q(退出) t(改类型)

# gdisk（GPT分区）
gdisk -l /dev/sda     # 列出GPT分区信息
gdisk /dev/sda        # 进入交互式分区工具

# partprobe 通知内核分区表已更改
partprobe /dev/sda    # 指定设备更新
partprobe             # 更新所有设备（有只读设备时可能报错）
```
...
```bash
# parted 支持 MBR 和 GPT，支持非交互式操作
parted /dev/sda print           # 打印分区信息
parted /dev/sda mklabel gpt     # 创建GPT分区表
parted /dev/sda mkpart primary xfs 1MiB 500MiB  # 创建分区
parted /dev/sda rm 1            # 删除第1个分区
...
mkfs.ext4 /dev/sda2           # 格式化为 EXT4
mkfs -t ext4 /dev/sda2        # 等价写法
mkfs.xfs -f /dev/sda1         # 强制格式化（覆盖已有文件系统）
mkfs.vfat /dev/sda1           # 格式化为 FAT32（用于EFI分区等）
```

- 格式化会清除分区上所有数据
- XFS 格式化时可指定参数优化性能：`mkfs.xfs -d su=64k,sw=4 /dev/md0`
- EXT4 格式化时可指定 inode 大小：`mkfs.ext4 -i 8192 /dev/sda1`
...
```bash
# 分区方式
mkswap /dev/sda3              # 将分区格式化为 swap

# 文件方式
dd if=/dev/zero of=/tmp/swap bs=1M count=1024  # 创建1G文件
mkswap /tmp/swap              # 格式化为 swap
chmod 0600 /tmp/swap          # 设置权限
```
...
```bash
swapon /dev/sda3              # 启用指定 swap 分区
swapon /tmp/swap              # 启用 swap 文件
swapon -s                     # 查看当前所有 swap 使用情况
swapon -a                     # 启用 /etc/fstab 中所有 swap

swapoff /dev/sda3             # 关闭指定 swap
swapoff -a                    # 关闭所有 swap
```
...
```bash
mknod /dev/mydevice c 240 0   # 创建字符设备，主设备号240，次设备号0
mknod /dev/myblock b 250 0    # 创建块设备
mknod /tmp/mypipe p           # 创建命名管道（FIFO）
```

- `c` 字符设备，`b` 块设备，`p` 命名管道
- 设备号可通过 `cat /proc/devices` 查看
- 一般由 udev 自动管理，手动创建较少见
...
```bash
tune2fs -l /dev/sda1                 # 查看文件系统超级块信息
tune2fs -c 30 /dev/sda1             # 设置30次挂载后强制fsck
tune2fs -i 180d /dev/sda1           # 设置180天后强制fsck
tune2fs -m 1 /dev/sda1              # 设置保留块比例为1%（默认5%）
tune2fs -L "mydata" /dev/sda1       # 设置卷标
tune2fs -j /dev/sda1                # 将ext2升级为ext3（添加日志）
tune2fs -O extents,uninit_bg /dev/sda1  # 启用ext4特性
```
...
```bash
sfdisk -d /dev/sda > sda_backup.txt     # 备份分区表
sfdisk /dev/sdb < sda_backup.txt        # 恢复/复制分区表到另一块盘
sfdisk -l /dev/sda                      # 列出分区信息
sfdisk --verify /dev/sda                # 验证分区表
```
...
sgdisk -L                               # 列出所有分区类型代码
sgdisk --backup=sda_gpt.bak /dev/sda    # 备份GPT分区表
sgdisk --load-backup=sda_gpt.bak /dev/sda  # 恢复分区表
sgdisk -R /dev/sdb /dev/sda             # 复制分区表到另一块盘
sgdisk -G /dev/sdb                      # 复制后需随机化GUID
sgdisk -Z /dev/sda                      # 清除GPT和MBR分区表
sgdisk -n 1:0:+512M -t 1:ef00 /dev/sda # 创建512M的EFI分区
```
...
gzip 仅能压缩单个文件，不能打包目录。压缩后原文件会被替换为 .gz 文件。

```bash
gzip file               # 压缩文件（原文件消失）
gzip -d file.gz         # 解压缩（等价于 gunzip file.gz）
gzip -k file            # 压缩但保留原文件
gzip -9 file            # 最高压缩比（1-9，默认6）
gzip -l file.gz         # 查看压缩文件信息

zcat file.gz            # 查看压缩文件内容（不解压）
...
bzip2 压缩比优于 gzip，但速度较慢。同样仅能压缩单个文件。

```bash
bzip2 file              # 压缩文件（原文件消失）
bzip2 -d file.bz2      # 解压缩（等价于 bunzip2 file.bz2）
bzip2 -k file           # 压缩但保留原文件
bzip2 -9 file           # 最高压缩比

bzcat file.bz2          # 查看压缩文件内容
bzgrep "pattern" file.bz2  # 在压缩文件中搜索
```

## 5.3. xz, xzcat/xzmore/xzless/xzgrep

xz 压缩比最高，但速度最慢、内存占用最大。Linux 内核源码使用 xz 压缩。

```bash
xz file                 # 压缩文件
xz -d file.xz          # 解压缩（等价于 unxz file.xz）
xz -k file             # 压缩但保留原文件
xz -9 file             # 最高压缩比
xz -T 4 file           # 使用4个线程压缩（多线程）

xzcat file.xz          # 查看压缩文件内容
xzgrep "pattern" file.xz  # 在压缩文件中搜索
```

## 5.4. tar

tar 本身是打包工具，配合压缩参数实现打包压缩。

```bash
# 打包压缩
tar -czf archive.tar.gz dir/      # gzip 压缩
tar -cjf archive.tar.bz2 dir/     # bzip2 压缩
tar -cJf archive.tar.xz dir/      # xz 压缩
...
XFS 文件系统专用的备份工具，支持增量备份。

```bash
xfsdump -l 0 -L "full_backup" -M "disk1" -f /backup/full.dump /srv/data
# -l 0    : level 0 为完整备份，1-9 为增量备份
# -L      : 备份标签
# -M      : 媒体标签
# -f      : 输出文件

xfsdump -I    # 查看已有的备份记录（/var/lib/xfsdump/inventory）
```

- 仅支持已挂载的 XFS 文件系统
- 不支持未挂载的文件系统或非 XFS 文件系统
- level 1-9 为增量备份，仅备份自上次同级或更低级别备份以来变更的文件
...
```bash
xfsrestore -f /backup/full.dump /srv/data     # 完整恢复
xfsrestore -f /backup/full.dump -L "full_backup" /srv/data  # 指定标签恢复
xfsrestore -I                                  # 查看备份信息
xfsrestore -f /backup/full.dump -s path/file /srv/data  # 恢复单个文件
```
...
创建 ISO 9660 文件系统镜像（也叫 genisoimage）。

```bash
mkisofs -o image.iso /path/to/dir              # 基本创建ISO
mkisofs -r -V "MY_LABEL" -o image.iso /path/   # -r 添加Rock Ridge扩展，-V 设置卷标
mkisofs -r -J -o image.iso /path/              # -J 添加Joliet扩展（Windows兼容）
```

- `-r` Rock Ridge 扩展，保留Linux权限信息
- `-J` Joliet 扩展，支持Windows长文件名
- `-V` 设置卷标
...
```bash
cdrecord -v dev=/dev/sr0 image.iso             # 刻录ISO到光盘
cdrecord -v dev=/dev/sr0 speed=4 image.iso     # 指定刻录速度
cdrecord --scanbus                             # 扫描可用刻录设备
cdrecord -v dev=/dev/sr0 blank=fast            # 快速擦除可重写光盘
```
...
cpio 用于创建和提取归档文件，常与 find 配合使用。也用于处理 initramfs。

```bash
# 创建归档
find /etc -print | cpio -o > /backup/etc.cpio
find . -name "*.conf" | cpio -ov > configs.cpio

# 提取归档
cpio -idv < /backup/etc.cpio
cpio -idv --no-absolute-filenames < etc.cpio  # 不使用绝对路径

# 查看归档内容
cpio -t < /backup/etc.cpio

# 配合 gzip 压缩
find /etc | cpio -o | gzip > /backup/etc.cpio.gz
gzip -dc /backup/etc.cpio.gz | cpio -idv
```
...
- `-d` 自动创建目录
- `-v` 显示详细信息
- 常见于 RPM 包内容提取：`rpm2cpio package.rpm | cpio -idv`
...
- 备份的是原始数据，包含未使用空间，文件较大
- 适合裸设备备份、磁盘克隆、MBR备份等场景

### 6.1.2. cpio

```bash
# 备份 /etc 目录
find /etc -print | cpio -ocvB > /backup/etc_backup.cpio

# 恢复
cd / && cpio -icvd < /backup/etc_backup.cpio
```

- `-B` 设置块大小为 5120 bytes，提高 I/O 效率
- 适合配合 find 做精确的文件筛选备份

### 6.1.3. tar

```bash
# 完整备份
tar -czpf /backup/full_$(date +%Y%m%d).tar.gz --exclude=/backup /

# 增量备份（基于时间戳文件）
tar -czpf /backup/incr_$(date +%Y%m%d).tar.gz \
    --newer-mtime="2024-01-01" /data

# 使用 --listed-incremental 进行增量备份
tar -czpf /backup/full.tar.gz --listed-incremental=/backup/snapshot.snar /data
tar -czpf /backup/incr1.tar.gz --listed-incremental=/backup/snapshot.snar /data
```
...
```bash
# 完整备份 XFS 文件系统
xfsdump -l 0 -L "full" -M "tape" -f /backup/xfs_full.dump /data

# 增量备份
xfsdump -l 1 -L "incr1" -M "tape" -f /backup/xfs_incr1.dump /data

# 恢复
xfsrestore -f /backup/xfs_full.dump /data
xfsrestore -f /backup/xfs_incr1.dump /data
```
...
```bash
# 本地同步
rsync -avz /source/ /dest/             # 同步目录（注意末尾/）
...
# 常用参数
rsync -avz --delete /src/ /dest/       # 删除目标端多余文件
rsync -avz --exclude='*.log' /src/ /dest/  # 排除文件
rsync -avzP /src/ /dest/              # 显示进度（-P = --partial --progress）
rsync -avz -e "ssh -p 2222" /src/ user@host:/dest/  # 指定SSH端口

# 用作备份
rsync -avz --backup --backup-dir=/backup/old /src/ /dest/
```
...
- `-z` 传输时压缩
- `--delete` 同步删除
- 源路径末尾带 `/` 表示目录内容，不带表示目录本身

## 6.3. timeshift

系统快照和恢复工具，类似 Windows 的系统还原。
...
# 恢复快照
timeshift --restore --snapshot '2024-01-01_00-00-01'

# 删除快照
timeshift --delete --snapshot '2024-01-01_00-00-01'
```

- 支持 RSYNC 和 BTRFS 两种快照模式
- BTRFS 模式利用文件系统自带的快照功能，效率更高
- RSYNC 模式通过硬链接实现增量备份
- 主要用于保护系统文件，不备份用户数据（可配置）
- 配置文件：`/etc/timeshift/timeshift.json`
...
```bash
tty                    # 显示当前终端设备名称，如 /dev/pts/0
who                    # 查看所有登录用户及其终端
```

- `/dev/tty` 当前终端
- `/dev/ttyN` 虚拟控制台（Alt+F1~F6 切换）
- `/dev/pts/N` 伪终端（SSH、终端模拟器等）
...
stty -a                        # 查看所有终端设置
stty size                      # 查看终端行列数
stty -echo                     # 关闭回显（适合密码输入场景）
stty echo                      # 恢复回显
stty erase '^H'                # 设置退格键
stty intr '^C'                 # 设置中断键
stty -icanon                   # 关闭规范模式（逐字符读取）
```

- 常用于脚本中处理密码输入、调整终端行为
- `stty sane` 可恢复终端到正常状态（终端显示异常时使用）

## 7.3. terminfo

terminfo 数据库描述终端的能力（如颜色数、光标移动序列等）。
...
```bash
tic myterm.ti          # 编译terminfo源文件并安装到数据库
tic -o /path myterm.ti # 指定安装目录
```
...
```bash
infocmp                # 显示当前终端的 terminfo 描述
infocmp xterm-256color # 显示指定终端类型的描述
infocmp xterm xterm-256color  # 比较两个终端类型的差异
```

### 7.3.3. tput

通过 terminfo 查询终端能力并执行操作。

```bash
tput colors            # 查看支持的颜色数
tput cols              # 查看终端列数
tput lines             # 查看终端行数
tput clear             # 清屏
...
```bash
tset                   # 初始化终端
tset -s                # 输出设置TERM变量的shell命令
reset                  # 重置终端到初始状态（等价于 tset -Q）
```
...
ncurses 是终端无关的字符界面库，基于 terminfo。

- 提供窗口、面板、菜单、表单等高级TUI组件
- 常见基于 ncurses 的工具：top, htop, vim, dialog, menuconfig
- 开发时链接：`gcc -o prog prog.c -lncurses`
- 相关头文件：`#include <ncurses.h>`
...
```bash
dircolors                      # 输出当前颜色配置的shell命令
dircolors -p                   # 打印默认的颜色数据库
dircolors -p > ~/.dircolors    # 导出配置以便自定义
dircolors ~/.dircolors         # 加载自定义配置
```

- 在 .bashrc 中加入 `eval "$(dircolors -b)"` 启用颜色
- 配置格式：`EXTENSION .log 01;33` （粗体黄色显示.log文件）
- 环境变量 `LS_COLORS` 保存当前颜色方案

## 7.5. /etc/inputrc 处理键盘映射

inputrc 是 readline 库的配置文件，控制命令行输入行为。

```bash
# 用户级配置：~/.inputrc
# 系统级配置：/etc/inputrc
# 生效方式：新开终端或 Ctrl+x Ctrl+r 重新读取

# 常见配置示例
set bell-style none              # 关闭响铃
set show-all-if-ambiguous on     # Tab补全时直接显示所有匹配
set completion-ignore-case on    # 补全忽略大小写
"\e[A": history-search-backward  # 上箭头按前缀搜索历史
"\e[B": history-search-forward   # 下箭头按前缀搜索历史
"\C-p": history-search-backward  # Ctrl+p 向前搜索
```

- readline 被 bash、python REPL、gdb 等大量程序使用
- `bind -p` 可查看当前所有键绑定
- `bind -f ~/.inputrc` 重新加载配置
...
```
用户名:密码占位:UID:GID:用户信息:主目录:默认shell
root:x:0:0:root:/root:/bin/bash
```

- 密码字段为 `x` 表示密码存放在 /etc/shadow
- UID 0 为 root，1-999 系统用户，1000+ 普通用户
- 可用 `vipw` 安全编辑此文件

### 8.1.2. /etc/shadow

存放用户加密密码和密码策略，仅 root 可读。

```
用户名:加密密码:最后修改日:最小间隔:最大有效期:警告天数:失效天数:帐号失效日:保留
root:$6$xxxx:18000:0:99999:7:::
```

- 密码字段以 `$算法id$salt$hash` 格式存储
  - `$1$` MD5, `$5$` SHA-256, `$6$` SHA-512
- 密码字段为 `!!` 或 `*` 表示账号被锁定
- 可用 `chage -l username` 查看密码策略
...
- 成员列表以逗号分隔
- 用户的初始组记录在 /etc/passwd 中，不在此列表中体现
- 可用 `vigr` 安全编辑
...
useradd -m -s /bin/bash username       # 创建用户并指定shell、创建主目录
useradd -u 1500 -g groupname username  # 指定UID和初始组
useradd -G wheel,docker username       # 指定附加组
useradd -r -s /sbin/nologin svcuser   # 创建系统用户（无登录权限）
useradd -e 2025-12-31 tempuser        # 设置账号过期日期
```

- `-m` 创建主目录（部分发行版默认创建）
- `-r` 创建系统用户（UID从SYS_UID_MIN分配）
- `-s` 指定默认shell
- `-d` 指定主目录路径
...
```bash
usermod -aG wheel username        # 追加附加组（-a 表示追加而非覆盖）
usermod -s /bin/zsh username      # 修改默认shell
usermod -d /new/home username     # 修改主目录（不移动文件）
usermod -d /new/home -m username  # 修改主目录并移动文件
usermod -l newname oldname        # 修改用户名
usermod -L username               # 锁定用户（密码前加!）
usermod -U username               # 解锁用户
usermod -e 2025-12-31 username    # 设置账号过期日
```
...
userdel -r username       # 删除用户及其主目录和邮件
userdel -f username       # 强制删除（即使用户已登录）
```

- 删除前建议先检查用户是否有正在运行的进程
- `find / -user username` 可查找该用户的所有文件
...
# 使用加密密码
echo 'user1:$6$salt$hash' | chpasswd -e
```
...
```bash
groupadd groupname            # 创建组（自动分配GID）
...
```bash
gpasswd -a user1 groupname   # 将用户加入组
gpasswd -d user1 groupname   # 将用户从组中移除
gpasswd -A user1 groupname   # 设置user1为组管理员
gpasswd groupname             # 设置组密码
gpasswd -r groupname          # 移除组密码
```
...
id -g username        # 仅显示主GID
id -Gn username       # 显示所有组名
```
...
chown --reference=ref_file target_file  # 参照另一文件
```
...
- 文件默认权限 = 666 - umask（文件默认不给执行权限）
- 目录默认权限 = 777 - umask
- umask 022：文件644，目录755
- umask 027：文件640，目录750
- 设置位置：/etc/profile 或 ~/.bashrc
...
# 设置 ACL
setfacl -m u:user1:rwx file          # 给指定用户设置权限
setfacl -m g:group1:rx file          # 给指定组设置权限
setfacl -m d:u:user1:rwx dir/        # 设置默认ACL（新建文件继承）
setfacl -x u:user1 file              # 移除指定用户的ACL
setfacl -b file                      # 移除所有ACL
setfacl -R -m u:user1:rx dir/        # 递归设置
...
- ACL 设置后，`ls -l` 权限字段末尾会出现 `+` 号
- mask 值限制了 ACL 中用户/组能获得的最大权限
...
Capability 将 root 的特权分割为多个独立的能力单元，可以单独赋予进程。
避免了给程序完整 root 权限带来的安全风险。

常见 Capability：

- `CAP_NET_BIND_SERVICE` 绑定小于1024的端口
- `CAP_NET_RAW` 使用原始套接字（如ping）
...
who                    # 查看当前登录用户
who -b                 # 查看最后一次系统启动时间
last                   # 查看登录历史记录（/var/log/wtmp）
last -n 10             # 最近10条登录记录
last reboot            # 查看重启历史
lastlog                # 每个用户最后一次登录时间
lastb                  # 查看登录失败记录（/var/log/btmp）
```
...
```bash
# 发送邮件
mail -s "subject" username     # 发送给本机用户，输入内容后Ctrl+D结束
echo "content" | mail -s "subject" username

# 查看邮件
mail                           # 查看当前用户邮箱
# 交互命令：h(列表) d(删除) r(回复) q(退出保存) x(退出不保存)
```

- 用户邮箱位置：`/var/spool/mail/username`
- 需要安装 mailx 或 mailutils
...
ulimit -a                      # 查看所有资源限制
ulimit -n                      # 查看最大文件描述符数
ulimit -n 65535                # 设置最大文件描述符数（当前会话）
ulimit -u                      # 查看最大进程数
ulimit -c unlimited            # 开启core dump（不限大小）
ulimit -s                      # 查看栈大小限制
```
...
```bash
ipcmk -M 1024                 # 创建1024字节的共享内存
ipcmk -Q                      # 创建消息队列
ipcmk -S 5                    # 创建含5个信号量的集合
```
...
```bash
ipcrm -m shmid                 # 删除共享内存（按ID）
ipcrm -M shmkey                # 删除共享内存（按key）
...
ipcs -l                        # 显示系统限制
ipcs -u                        # 显示使用摘要
ipcs -p                        # 显示创建者PID和最后操作PID
```

## 9.3. CGroup

Control Groups，限制和隔离进程组的资源使用（CPU、内存、I/O等）。
...
- cgroup v1 按资源类型分层（cpu, memory, blkio 等）
- cgroup v2 统一层次结构（推荐）
- Docker/K8s 通过 cgroup 实现容器资源限制
...
- job number 和 PID 不同，kill 后台任务需要加 `%`
- `+` 标记当前默认任务，`-` 标记上一个任务

### 12.1.2. nohup

使命令忽略 SIGHUP 信号，在终端关闭后继续运行。

```bash
nohup command &                    # 后台运行，输出到 nohup.out
nohup command > /dev/null 2>&1 &   # 丢弃所有输出
nohup command > output.log 2>&1 &  # 输出到指定文件
```
...
kill -HUP PID                  # 重新读取配置（如nginx）
kill -STOP PID                 # 暂停进程
kill -CONT PID                 # 继续运行

killall nginx                  # 按名称杀死所有匹配进程
killall -9 python              # 强制杀死所有python进程
killall -u username            # 杀死某用户的所有进程
killall -I Name                # 忽略大小写匹配

pkill -f "pattern"             # 按命令行模式匹配并杀死
```
...
ps -o pid,ppid,cmd,%mem,%cpu -p PID  # 自定义输出列
ps --forest                    # 树形显示
ps -eo pid,ppid,cmd --sort=-%mem | head  # 按内存排序

pstree                         # 树状显示进程
pstree -p                      # 显示PID
pstree -p PID                  # 显示指定进程的子进程树
pstree -u                      # 显示用户名
```
...
```bash
pgrep nginx                    # 按名称查找进程PID
pgrep -f "python script.py"   # 按完整命令行匹配
pgrep -u root sshd            # 指定用户的进程
pgrep -l nginx                # 同时显示进程名
pgrep -a nginx                # 显示完整命令行

pidof nginx                    # 获取程序的所有PID
pidof -s nginx                 # 仅返回一个PID
```
...
top                            # 实时查看进程状态
top -p PID                     # 监控指定进程
top -u username                # 仅显示指定用户进程
top -d 2                       # 刷新间隔2秒
top -bn1                       # 批处理模式，输出一次（适合脚本）
```
...
```bash
nice -n 10 command             # 以较低优先级运行（-20到19，值越大优先级越低）
nice -n -5 command             # 以较高优先级运行（需要root）

renice 10 -p PID              # 修改运行中进程的优先级
renice -5 -p PID              # 提升优先级（需要root）
renice 10 -u username         # 修改用户所有进程优先级
```

- 默认 nice 值为 0
- 普通用户只能降低优先级（增大nice值）
- root 可以设置 -20 到 19 的任何值
...
- `procs`: r(运行队列) b(阻塞进程)
- `memory`: swpd(虚拟内存使用) free(空闲) buff(缓冲) cache(缓存)
- `swap`: si(换入) so(换出)，持续非零说明内存不足
- `io`: bi(块设备读) bo(块设备写)
- `system`: in(中断数) cs(上下文切换)
- `cpu`: us(用户态) sy(内核态) id(空闲) wa(IO等待) st(被虚拟机偷走)
...
```bash
fuser -uv .                    # 查看当前目录被哪些进程使用
fuser -muv .                   # 包含挂载点信息
fuser -v /dev/sda1             # 查看设备被哪些进程使用
fuser -k /mnt/data             # 杀死所有使用该挂载点的进程
fuser -v 80/tcp                # 查看占用80端口的进程
```

- 常用于 umount 提示 busy 时，查找并处理占用进程
...
```bash
lsof /path/to/file             # 查看谁在使用某文件
lsof -i :80                    # 查看占用80端口的进程
lsof -i tcp                    # 查看所有TCP连接
lsof -u username               # 查看用户打开的文件
lsof -p PID                    # 查看进程打开的文件
lsof -c nginx                  # 查看nginx进程打开的文件
lsof +D /var/log/              # 查看目录下被打开的文件（递归）
```

### 12.6.3. pidof

```bash
pidof nginx                    # 获取nginx的所有PID
pidof -s sshd                  # 仅返回一个PID
```
...
- Stage 1：存放在 MBR 的前 446 字节中
  - 功能极简，仅负责加载 Stage 1.5 或 Stage 2
  - 因为空间太小，无法包含文件系统驱动

- Stage 1.5（可选）：存放在 MBR 与第一个分区之间的空间
  - 包含文件系统驱动，使 Stage 2 可以从文件系统中加载
  - 位于第一个扇区之后、第一个分区之前（通常为63个扇区）

- Stage 2：存放在 /boot/grub2/ 目录中
  - 提供完整的开机菜单界面
  - 能够读取配置文件、加载内核和 initramfs
  - 支持命令行交互
...
# 设置默认启动项
grub2-set-default 0                    # 设置第一个菜单项为默认
grub2-set-default "CentOS Linux"       # 按名称设置
```

### 15.5.3. grub2 配置文件维护 /etc/default/grub 与 /etc/grub.d

`/etc/default/grub` 主要参数：

```bash
GRUB_TIMEOUT=5                         # 菜单等待时间（秒）
GRUB_DEFAULT=saved                     # 默认启动项
GRUB_CMDLINE_LINUX="crashkernel=auto"  # 传递给内核的参数
GRUB_DISABLE_RECOVERY="true"           # 是否禁用恢复模式菜单
```

`/etc/grub.d/` 目录下的脚本按编号顺序执行：

- `00_header` 基本设置
- `10_linux` 自动检测Linux内核
...
# 添加额外模块到 initramfs
# 编辑 /etc/dracut.conf 或 /etc/dracut.conf.d/*.conf
# add_drivers+=" module_name "
```

- 更换磁盘控制器、调整存储架构后需要重建 initramfs
- 内核更新时通常会自动重建
...
- 忘记 root 密码
  - 在 grub2 菜单按 `e` 编辑启动项
  - 在 linux 行末尾添加 `rd.break` 或 `init=/bin/bash`
  - 进入紧急模式后：`mount -o remount,rw /sysroot && chroot /sysroot && passwd root`
  - 如果有 SELinux：`touch /.autorelabel`

- 文件系统损坏无法开机
  - 进入救援模式（rescue.target）
  - `fsck /dev/sdXN` 修复文件系统（XFS用 `xfs_repair`）
  - 注意：修复前必须 umount
...
- 开机卡在某个服务
  - 开机时在 grub 菜单添加 `systemd.unit=rescue.target`
  - 进入后禁用问题服务：`systemctl disable problem.service`
...
- Core(s) per socket：每颗CPU的核心数
- Socket(s)：物理CPU数量
- 总逻辑CPU = Socket x Core x Thread
...
- 需要 `pciutils` 包
- 设备ID数据库：`/usr/share/hwdata/pci.ids`
...
lsusb -v                       # 详细信息
lsusb -t                       # 树状显示USB总线拓扑
lsusb -d vendor:product        # 按ID查找设备
```
...
```bash
iostat                         # 基本CPU和磁盘统计
iostat -x 1 5                  # 扩展统计，每秒刷新，共5次
iostat -d -k                   # 仅磁盘统计，KB为单位
iostat -p sda                  # 指定设备
```

关键指标：

- `%util` 设备繁忙比例（接近100%说明I/O瓶颈）
...
telnet 192.168.1.1 80          # 测试HTTP端口
telnet 192.168.1.1 3306        # 测试MySQL端口
```

- 传输明文，不安全，已被 SSH 替代
- 目前主要用于快速测试 TCP 端口连通性
- 退出方式：`Ctrl+]` 后输入 `quit`
...
ping -s 1400 host              # 设置包大小（测试MTU）
ping -W 2 host                 # 设置超时2秒
ping -I eth0 host              # 指定出接口
```
...
host -t AAAA example.com       # 查询IPv6地址
host 8.8.8.8                   # 反向解析
```

- 类似工具：`dig`（更详细）、`nslookup`（交互式）
...
```bash
traceroute host                # 追踪路由（UDP方式）
traceroute -I host             # 使用ICMP
traceroute -T host             # 使用TCP
traceroute -p 443 host         # 指定端口
traceroute -m 15 host          # 最大跳数

# mtr：ping + traceroute 的结合
mtr host                       # 实时显示路由和丢包
mtr -r -c 10 host             # 报告模式，发送10个包
```

### 17.2.3. iwd

Intel 开发的无线网络管理工具，用于替代 wpa_supplicant。
...
#   station wlan0 connect SSID      # 连接网络
#   station wlan0 disconnect        # 断开
...
- 配置文件：`/var/lib/iwd/`
- 服务：`systemctl enable --now iwd`
- 相比 wpa_supplicant 更轻量，启动更快

### 17.2.4. ip

iproute2 套件的核心命令，替代 ifconfig/route/arp 等。

```bash
# 地址管理
ip addr show                   # 查看所有网卡地址（等价 ip a）
...
ss -tunlp                      # 查看所有监听的TCP/UDP端口
ss -t state established        # 查看已建立的TCP连接
ss -s                          # 显示socket统计摘要
ss -o state time-wait          # 查看TIME_WAIT状态连接
ss dst 192.168.1.100           # 按目标地址过滤
ss sport = :80                 # 按源端口过滤
```

- `-t` TCP, `-u` UDP, `-n` 数字格式, `-l` 监听, `-p` 进程信息
- 比 netstat 更快，信息更丰富

### 17.2.6. network manager

NetworkManager 是常见的网络配置管理服务。

```bash
# nmcli 命令行工具
nmcli device status            # 查看设备状态
nmcli connection show          # 查看所有连接
nmcli connection up "conn_name"    # 激活连接
nmcli connection down "conn_name"  # 断开连接
...
Linux 虚拟网络是容器和虚拟化技术的基础。

### 17.3.1. namespace

网络命名空间提供网络栈的隔离（接口、路由表、iptables等各自独立）。
...
ip netns exec ns1 bash         # 进入命名空间的shell

# 删除命名空间
ip netns delete ns1
```

- 容器技术（Docker, K8s）的网络隔离基于 network namespace
- 每个 namespace 有独立的网卡、路由表、iptables 规则

### 17.3.2. tap/tun

虚拟网络设备，用于用户态程序和内核网络栈交互。
...
- **tun** 工作在第三层（IP层），处理IP包 -- 用于VPN（如OpenVPN）
- **tap** 工作在第二层（链路层），处理以太网帧 -- 用于虚拟机桥接网络

### 17.3.3. veth

虚拟以太网对（Virtual Ethernet Pair），总是成对出现，一端发送的数据另一端接收。
...
- 常用于连接两个 network namespace
- Docker 容器网络的基础：一端在容器内，另一端连接到网桥

### 17.3.4. brctl (bridge)

Linux 网桥，工作在第二层，相当于虚拟交换机。

```bash
# 旧版工具 brctl（bridge-utils包）
brctl addbr br0                # 创建网桥
brctl addif br0 eth0           # 添加接口到网桥
brctl addif br0 veth0          # 添加veth到网桥
brctl show                     # 查看网桥信息
brctl delif br0 eth0           # 从网桥移除接口
...
ip link set br0 up
bridge link show               # 查看桥接信息
```

- Docker 默认的 docker0 就是一个 Linux bridge
- 多个容器的 veth 对连接到同一网桥实现互通

### 17.3.5. iptables

Linux 内核防火墙，工作在网络层，实现包过滤、NAT、端口转发等。

```bash
# 查看规则
iptables -L -n -v              # 查看filter表
iptables -t nat -L -n          # 查看NAT表

# 允许/拒绝
iptables -A INPUT -p tcp --dport 22 -j ACCEPT    # 允许SSH
iptables -A INPUT -p tcp --dport 80 -j DROP      # 拒绝HTTP
iptables -A INPUT -s 192.168.1.0/24 -j ACCEPT   # 允许子网

# NAT
iptables -t nat -A POSTROUTING -o eth0 -j MASQUERADE  # 源地址转换
iptables -t nat -A PREROUTING -p tcp --dport 8080 \
  -j DNAT --to-destination 10.0.0.2:80            # 端口转发
...
pacman -S --needed pkg1 pkg2   # 安装（已装则跳过）

# 卸载
pacman -R package              # 卸载
pacman -Rs package             # 卸载及未使用的依赖

# 查询
pacman -Ss keyword             # 搜索包
pacman -Qi package             # 查看已安装包的信息
pacman -Ql package             # 查看包安装了哪些文件
...
# 常用命令
# r (run)        运行程序
# b main (break) 在main设断点
# b file.c:20    在文件第20行设断点
# n (next)       单步执行（不进入函数）
# s (step)       单步执行（进入函数）
# c (continue)   继续运行
# p var (print)  打印变量值
...
```bash
ldd /usr/bin/ls                # 查看ls依赖的共享库
ldd -v /usr/bin/ssh            # 详细显示版本信息
ldd -r libfoo.so               # 检查未解析的符号
...
- 输出中 `not found` 表示缺少依赖
- `LD_LIBRARY_PATH` 可指定额外的库搜索路径
- `/etc/ld.so.conf` 和 `ldconfig` 管理系统级库搜索路径
...
打印运行中进程的所有线程调用栈（底层调用 gdb）。

```bash
pstack PID                     # 打印进程所有线程的调用栈

# 等价操作
gdb -batch -ex "thread apply all bt" -p PID
```

- 常用于排查进程挂死（deadlock）问题
- 多次执行对比可发现进程卡在哪里
- 需要目标程序有调试符号才能看到函数名
...
strace -f command              # 同时跟踪子进程
strace -e trace=open,read,write command  # 仅跟踪指定调用
strace -e trace=network command    # 仅跟踪网络相关调用
strace -c command              # 统计每个系统调用的时间和次数
strace -o output.txt command   # 输出到文件
strace -t command              # 显示时间戳
strace -T command              # 显示每个调用的耗时
```

- 常用类别：`-e trace=file`(文件), `network`(网络), `process`(进程), `signal`(信号)
- 排查"Permission denied"、"No such file"等问题非常有效
- ltrace 类似但跟踪的是库函数调用

## 19.6. ipcs 查询进程间通信状态

查看 System V IPC 资源（共享内存、消息队列、信号量）。
...
iostat -xd 2 5                 # 仅磁盘，每2秒输出，共5次
iostat -p sda                  # 查看指定磁盘及其分区
```

- `%util` 接近 100% 说明磁盘I/O饱和
- `await` > 10ms 通常表示I/O较慢
- 详见 16.6 节

## 19.9. sar 找出系统瓶颈的利器

系统活动报告工具，可收集和报告系统各方面的性能数据。

```bash
sar -u 1 5                     # CPU使用率，每秒1次，共5次
sar -r 1 5                     # 内存使用率
sar -b 1 5                     # I/O统计
...
# 查看历史数据（需要开启sysstat服务）
sar -u -f /var/log/sa/sa20     # 查看20号的CPU数据
sar -r -s 08:00:00 -e 12:00:00  # 指定时间段
```

- 需要安装 `sysstat` 包并启用 `sysstat.service`
- 数据保存在 `/var/log/sa/` 目录
- 综合性能分析的首选工具

## 19.10. readelf elf 文件格式分析

分析 ELF 格式的可执行文件、目标文件和共享库。

```bash
readelf -h binary              # 显示ELF文件头
readelf -l binary              # 显示程序头表（段信息）
readelf -S binary              # 显示节头表
readelf -s binary              # 显示符号表
readelf -d binary              # 显示动态段（依赖库等）
readelf -r binary              # 显示重定位信息
readelf -n binary              # 显示NOTE段
readelf --debug-dump binary    # 显示调试信息
```

- ELF: Executable and Linkable Format
- 比 objdump 更专注于 ELF 结构分析
...
objdump -D binary              # 反汇编所有段
objdump -S binary              # 混合源码和汇编（需-g编译）
objdump -t binary              # 显示符号表
objdump -h binary              # 显示段头信息
objdump -R binary              # 显示动态重定位
...
nm binary                      # 列出所有符号
nm -D binary                   # 仅列出动态符号
nm -u binary                   # 仅列出未定义的符号（外部依赖）
nm -g binary                   # 仅列出全局符号
nm -C binary                   # C++符号demangle
nm --size-sort binary          # 按大小排序
nm -A *.o | grep func_name    # 在多个文件中搜索符号
```
...
```bash
size binary                    # 显示各段大小
size -A binary                 # System V 格式（更详细）
size -B binary                 # Berkeley 格式（默认）
size *.o                       # 查看多个目标文件
```
...
```bash
tcpdump -i eth0                # 抓取指定网卡的包
tcpdump -i any                 # 抓取所有网卡
tcpdump -i eth0 port 80       # 过滤端口
tcpdump -i eth0 host 192.168.1.1    # 过滤主机
tcpdump -i eth0 src 10.0.0.1  # 过滤源地址
tcpdump -i eth0 dst port 443  # 过滤目标端口
tcpdump -i eth0 tcp            # 仅TCP
tcpdump -i eth0 -c 100        # 抓100个包后停止
tcpdump -i eth0 -w capture.pcap  # 写入文件（可用Wireshark打开）
tcpdump -r capture.pcap        # 读取pcap文件
tcpdump -nn -i eth0            # 不解析主机名和端口名
tcpdump -A -i eth0 port 80    # 以ASCII显示内容
tcpdump -X -i eth0 port 80    # 以HEX和ASCII显示
```

- `-nn` 不做DNS和服务名解析（更快）
- `-v/-vv/-vvv` 增加详细程度
- 组合过滤：`tcpdump -i eth0 'tcp port 80 and host 10.0.0.1'`
